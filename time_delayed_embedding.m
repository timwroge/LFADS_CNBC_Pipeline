...
for i = 1:numTaskConditions
    for j = 1:numTrials
        if(taskAngles(i) == taskConditions(j))
            factor = factors(1, : ,j);
            x = factor(1:end-2*tau);
            y = factor(tau+1:end-tau);
            z = factor(2*tau + 1:end);
            hold on, plot3(x, y, z, 'Color', colorMapping(i, :));
         end
     end
end
xlabel('Factor 1 ');
ylabel('Factor 1 - Tau');
zlabel('Factor 1 - 2*Tau');
title('Time Delayed Embedding of Dynamics');
disp('Saving figure' )
savefig('time_delayed_embedding.fig');
disp('Figure saved' )
